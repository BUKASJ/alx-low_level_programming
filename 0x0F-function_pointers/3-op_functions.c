#include "3-calc.h"
#include <stdio.h>
#include <stdlib.h>

/**
 * op_add - addition operation
 * @a: the number 'a' to be added
 * @b: the number 'b'to be added
 *
 * Return: the sum of a and b
 */

int op_add(int a, int b)
{
	return (a + b);
}

/**
 * op_sub - subtraction operation
 * @a: the number 'a' to be subtracted
 * @b: the number 'b' to be subtracted
 *
 * Return: returns the difference of a and b
 */

int op_sub(int a, int b)
{
	return (a - b);
}

/**
 * op_mul - multiplication operation
 * @a: the number 'a' to be multiplied
 * @b: the number 'b' to be multiplied
 *
 * Return: returns the product of a and b
 */

int op_mul(int a, int b)
{
	return (a * b);
}

/**
 * op_div - division operation
 * @a: the number to be divided
 * @b: the divisor
 *
 * Return: returns the result of the division of a by b
 */

int op_div(int a, int b)
{
	if (b == 0)
	{
		printf("Error\n");
		exit(100);
	}
	return (a / b);
}

/**
 * op_mod - modulo operation
 * @a: the number to be divided
 * @b: the divisor
 *
 * Return: returns the remainder of the division of a by b
 */

int op_mod(int a, int b)
{
	if (b == 0)
	{
		printf("Error\n");
		exit(100);
	}
	return (a % b);
}
